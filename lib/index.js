...
function BrowserWindow(options) {
  ...
  browserWindow.on('closed', function() {
    ...
    // 同时清理缓存的 browserWindow 对象
    threadDictionary.removeObjectForKey(identifier + '.browserWindow')
  ...
  // 同时缓存 browserWindow 对象本身，用于 fromId 方法直接返回
  // 这样可以避免每次调用 fromId 时创建新的 EventEmitter，导致 postMessage 消息丢失
  threadDictionary[identifier + '.browserWindow'] = browserWindow
...
BrowserWindow.fromId = function(identifier) {
  ...
  // 优先返回缓存的 browserWindow 对象，保证是同一个 EventEmitter 实例
  // 这样 setDelegates 中注册的事件处理器可以正确工作
  if (threadDictionary[identifier + '.browserWindow']) {
    return threadDictionary[identifier + '.browserWindow']
  }

  // 向后兼容：如果只有 panel 缓存（旧版本创建的窗口），则使用 fromPanel
